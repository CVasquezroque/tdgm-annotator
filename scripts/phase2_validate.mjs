...
const userAdminSource = read('src/services/userAdmin.ts')
...
assert(sessionService.includes('saveReviewedSessionSegments'), 'Phase 2 reviewer segment edit save missing')
assert(sessionService.includes('saveReviewSessionNotes'), 'Phase 2 review note save missing')
assert(sessionService.includes('deleteAnnotationSession'), 'Phase 2 complete annotation deletion missing')
...
assert(app.includes('DashboardPanel'), 'App missing dashboard panel')
...
assert(rules.includes('reviewerUpdatesSubmittedSegment'), 'reviewers should only edit existing submitted segments')
assert(rules.includes('reviewerUpdatesSegmentNotes'), 'reviewers should be able to update only segment notes from view mode')
assert(rules.includes('request.auth.token.get("admin", false)'), 'admin claim lookup should tolerate missing custom claims')
assert(rules.includes('allow delete: if parentEditableByOwner() || isAdmin()'), 'admins should delete nested session segments')
assert(rules.includes('allow delete: if isAdmin();'), 'admins should have explicit destructive delete permissions')
...
assert(sessionService.includes('listSessionsForDashboard'), 'annotationSessions.ts should list sessions for dashboard summaries')
assert(userAdminSource.includes('deleteUserProfileAdmin'), 'userAdmin.ts should expose admin profile deletion')
